 ...
 ***************************************************************************/
static int32_t
zs_store_process (ZIPstream *zstream, ZIPentry *zentry, uint8_t *entry, int64_t entrySize,
...
zs_deflate_init (ZIPstream *zstream, ZIPentry *zentry)
{
  ...
  if (deflateInit2 (zlstream, Z_DEFAULT_COMPRESSION, Z_DEFLATED, -MAX_WBITS, DEF_MEM_LEVEL,
 ...
 ***************************************************************************/
static int32_t
zs_deflate_process (ZIPstream *zstream, ZIPentry *zentry, uint8_t *entry, int64_t entrySize,
                    int64_t *entryConsumed, uint8_t *writeBuffer, int64_t writeBufferSize)
{
  z_stream *zlstream;
  int flush;
  ...
  if (!(entry && rv == Z_OK) && !(!entry && rv == Z_STREAM_END))
  {
    ...
    if (rv == Z_BUF_ERROR)
      fprintf (
          stderr,
          "  ZLIB: Z_BUF_ERROR, No progress is possible; either avail_in or avail_out was zero\n");
    else if (rv == Z_MEM_ERROR)
      fprintf (stderr, "  ZLIB: Z_MEM_ERROR, Insufficient memory\n");
    else if (rv == Z_STREAM_ERROR)
      fprintf (stderr,
               "  ZLIB: Z_STREAM_ERROR, The stream state is inconsistent or stream was NULL\n");
    else
 ...
 ***************************************************************************/
ZIPmethod *
zs_registermethod (ZIPstream *zs, int32_t methodID, int32_t (*init) (ZIPstream *, ZIPentry *),
                   int32_t (*process) (ZIPstream *, ZIPentry *, uint8_t *, int64_t, int64_t *,
                                       uint8_t *, int64_t),
 ...
 ***************************************************************************/
ZIPentry *
zs_writeentry (ZIPstream *zstream, uint8_t *entry, int64_t entrySize, char *name, time_t modtime,
               int methodID, int64_t *writestatus)
{
 ...
 ***************************************************************************/
ZIPentry *
zs_entrydata (ZIPstream *zstream, ZIPentry *zentry, uint8_t *entry, int64_t entrySize,
              int64_t *writestatus)
{
  int32_t writeSize = 0;
  int64_t lwritestatus;
  ...
  while ((writeSize = zentry->method->process (zstream, zentry, entry, remaining, &consumed,
                                               zstream->buffer, sizeof (zstream->buffer))) > 0)
  {
...
zs_entryend (ZIPstream *zstream, ZIPentry *zentry, int64_t *writestatus)
{
  ...
  if (zentry->method->finish && zentry->method->finish (zstream, zentry))
  {
...
zs_finish (ZIPstream *zstream, int64_t *writestatus)
{
  ...
  while (zentry)
  {
    ...
    zs_packunit32 (zstream, &packed,
                   (zip64) ? 0xFFFFFFFF
                           : zentry->LocalHeaderOffset); /* Relative offset of Local Header */
  ...
  if (zip64eocd)
  {
    ...
    /* Write ZIP64 End of Central Directory Record, packing into write buffer and swapped to
     * little-endian order */
    packed = 0;
    ...
    zs_packunit64 (zstream, &packed,
                   zstream->CentralDirectoryOffset); /* Offset to Central Directory */
    ...
    /* Write ZIP64 End of Central Directory Locator, packing into write buffer and swapped to
     * little-endian order */
    packed = 0;
...
zs_writedata (ZIPstream *zstream, uint8_t *writeBuffer, int64_t writeBufferSize)
{
  ...
  {
    writeLength =
        ((writeBufferSize - written) > ZS_WRITE_SIZE) ? ZS_WRITE_SIZE : (writeBufferSize - written);
...
zs_htolx (void *data, int size)
{
  ...
  if (le == 0)
  {
    ...
    case 4:
      data4 = (uint32_t *)data;
      *data4 = (((*data4 >> 24) & 0xff) | ((*data4 & 0xff) << 24) | ((*data4 >> 8) & 0xff00) |
                ((*data4 & 0xff00) << 8));
      break;
    case 8:
      data4 = (uint32_t *)data;
      ...
      h0 = data4[0];
      h0 =
          (((h0 >> 24) & 0xff) | ((h0 & 0xff) << 24) | ((h0 >> 8) & 0xff00) | ((h0 & 0xff00) << 8));

      h1 = data4[1];
      h1 =
          (((h1 >> 24) & 0xff) | ((h1 & 0xff) << 24) | ((h1 >> 8) & 0xff00) | ((h1 & 0xff00) << 8));
